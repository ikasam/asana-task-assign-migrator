...
main への push を起点に GitHub Release を自動化する。要件 [R26〜R31](01_requirements.md) を満たす。実装では [release.yml](../../.github/workflows/release.yml) を main push の **caller**、[release-build.yml](../../.github/workflows/release-build.yml) を **reusable**（品質ゲート → tag → build → publish）に分離した（旧 `on: push: tags` の release.yml はこの caller に置き換え）。[ci.yml](../../.github/workflows/ci.yml) は従来どおり push(main) / PR で動く。
...
- 素朴な「main-push workflow が `GITHUB_TOKEN` で tag を push → `on: push: tags` の別 workflow が起動」は **成立しない**（GitHub の再帰防止。tag は作られるが当該 workflow は沈黙）。本実装はこの連鎖に頼らず、release.yml(caller) が release-build.yml(reusable) を同一 run で `uses:` する。
