...
1. **`GITHUB_TOKEN` で push した tag は `push:tags` 連鎖を起こさない**（[C-019](05_constraints.md)）。だから「tag push → release.yml(`push:tags`) 起動」には頼らず、**同一 run で reusable workflow を直接呼ぶ**。例外は `workflow_dispatch`/`repository_dispatch`（将来の代替手段）。
...
5. **既存 [release.yml](../../.github/workflows/release.yml)（`on: push: tags`）の扱い**: reusable 化して caller から呼ぶ。`on: push: tags` を手動 fallback として残すかは実装時判断。残すなら R31 の idempotency で二重リリースを防ぐ。
