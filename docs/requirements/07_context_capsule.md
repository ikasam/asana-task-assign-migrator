...
- ドメインのときは `workspacesApi.getWorkspaces({opt_fields:"name,is_organization,email_domains"})` の `email_domains` 照合で GID に解決（S-027）。0 件 / 複数件は exit 2 + 可視 workspace 列挙（R25）。
...
3. **`email_domains` の返却は要検証（H-API8）+ PAT 可視性リスク（C-018 / C-017 と同種）**。実装初手で 1-call spike（`GET /workspaces?opt_fields=name,is_organization,email_domains`）。返らなければドメイン解決を断念し GID 必須へ戻す（C-014 維持）。
