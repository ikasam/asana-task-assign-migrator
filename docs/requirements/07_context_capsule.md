...
3. **`email_domains` の返却は 2026-05-30 実機検証で採択済（H-API8）**。検証 PAT では複数 organization がいずれも `email_domains` 非空で返り、指定ドメインが正しい単一 gid に解決（兄弟 org とも誤マッチせず）。**ただし C-018（PAT 可視性で空のこともある / 素の workspace は非対象）は別 PAT・別環境では依然リスク**なので、`workspaceListHint` の fallback（GID 直指定への案内）と「返らなければ GID 必須へ戻す」方針は維持する。
...
6. **workspace-scoped API には raw `args.workspace` でなく `resolveWorkspace` が返す `workspace.gid` を渡す** (S-027 下位制約): `--workspace` は domain にもなり得る (R24) ため `args.workspace` は raw 入力 (gid | domain)。`getUserForWorkspace` / `listWorkspaceUsers` / `getTasks` 等に raw を渡すと domain 指定時に実行時破綻する。migrate は `workspace.gid` 使用で正しかったが、PR #5 R1 で survey が raw を渡して破綻し修正済み。**新たな workspace-scoped 呼び出しを足すときも必ず解決済み gid を使う** (入力の表現が gid → 解決値に変わった結果の representation-normalization 不変条件)。
...
## 追加フェーズ: リリース自動化（2026-05-31）

main への push を起点に GitHub Release を自動化する要求分析を実施。実装フェーズへの引き継ぎ要約。

### 確定事項（リリース自動化）

- **トリガー条件**: main への push のうち deno.json の `.version` が直前から変化した push のみリリース（判断 31-A）。`github.event.before..after` の version 比較。不変は no-op（R28）。
- **連携方式**: tag 作成 → build+publish を**同一 workflow run** で完結（判断 32-A）。`workflow_call` の reusable workflow を呼ぶ。`GITHUB_TOKEN` の tag push が他 workflow を非トリガーにする制約（[C-019](05_constraints.md)）を構造的に回避。**secret 不要**。
- **version single source**: deno.json を source とし、src/cli.ts は `import ... with { type: "json" }` で導出（判断 33-A、二重定義 [C-020](05_constraints.md) を解消）。bump は機能追加/バグ修正時の運用規約（CI 強制はしない＝判断 35-A）。
- **品質ゲート**: release 経路で build 前に `fmt:check`/`lint`/`check`/`test` を実行、失敗で中止（判断 34-B）。tag は test 通過後に push し dangling tag を残さない。
- **idempotency**: 同名 tag/release が既存なら fail（loud）、上書きしない（R31）。再リリースは version bump を要する。

### 後工程で忘れると危険な文脈（リリース自動化）

1. **`GITHUB_TOKEN` で push した tag は `push:tags` 連鎖を起こさない**（[C-019](05_constraints.md)）。だから「tag push → `on: push: tags` workflow 起動」には頼らず、**同一 run で reusable workflow を直接呼ぶ**（実装: release.yml=caller → release-build.yml=reusable を `uses:`）。例外は `workflow_dispatch`/`repository_dispatch`（将来の代替手段）。
2. **順序の不変条件**: test pass → tag push → build → `gh release create --verify-tag`。tag を build/publish 前に remote へ push しないと `--verify-tag` が落ちる。test を tag 作成より前に置かないと、test 失敗時に dangling tag が残る（[S-030](03_specifications.md)）。
3. **version 検出は before..after の端点比較**。複数 commit を含む push でも端点だけ見る（中間 commit の version 揺れは無視）。force-push / `before` 欠落時は安全側 no-op（[S-028](03_specifications.md)）。
4. **cli.ts の VERSION ハードコードを廃止して deno.json から読む**。compile 同梱は [H-DENO4](02_hypotheses.md) で要検証（偽なら build 時 codegen にフォールバック、single source = deno.json は維持）。`--version` 出力と tag 名が deno.json と一致すること（R29）。
5. **workflow の最終構成**: [release.yml](../../.github/workflows/release.yml) = main push の caller（version 変化検知）、[release-build.yml](../../.github/workflows/release-build.yml) = reusable（品質ゲート → tag → build → publish）。旧 `on: push: tags` の手動 fallback は**残していない**（手動リリースは deno.json の version bump で行う）。
6. **bump 忘れは安全側に倒れる**（リリースが出ないだけ）。CI 強制（判断 35-B/C）は誤検知が多いため見送り。頻発したら昇格。

### 要検証事項（リリース自動化）

| ID | 内容 | 状態 |
|---|---|---|
| H-DENO4 | `import denoJson from "../deno.json" with { type: "json" }` の version が `deno compile` バイナリに同梱され `--version` に反映される | ✓ 採択（2026-05-31、compile した binary の `--version` が deno.json と一致。cli.ts は worker が編集、司令塔が pinned toolchain で check/lint/fmt/test 45 pass + compile を検証） |

### リリース自動化の参照

- 要望 [W-010](01_requirements.md) / 要求 [D-019〜D-022](01_requirements.md) / 要件 [R26〜R31](01_requirements.md) / 仕様 [S-028〜S-032](03_specifications.md) / 判断 [31〜35](06_decisions.md) / 設計選択 [12〜14](04_design_options.md) / 制約 [C-019, C-020](05_constraints.md) / 仮説 [H-DENO4](02_hypotheses.md)
