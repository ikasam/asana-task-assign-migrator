...
| ID | 内容 | 状態 |
|---|---|---|
| H-DENO4 | `import deno.json with {type:"json"}` の version が `deno compile` バイナリに同梱され `--version` に反映される | ✓ 採択（2026-05-31、compile した binary の `--version` が deno.json と一致。cli.ts は worker が編集、司令塔が pinned toolchain で check/lint/fmt/test 45 pass + compile を検証） |
