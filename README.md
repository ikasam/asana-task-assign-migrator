...
依存ライブラリ（`npm:asana` ほか）はいずれも permissive ライセンス（MIT / ISC / BSD / Apache-2.0）で、コンパイル済みバイナリへの同梱・再配布が可能です。第三者ライブラリの帰属表示は [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) にまとめています。バイナリを配布する場合はこのファイルを同梱してください。
