...
GitHub Actions は **コミット SHA で pin** しています（タグ移動によるサプライチェーン攻撃対策。バージョンは末尾コメントで追跡）。新しい Action を追加したら `mise run actions:pin` で pin してください。CI の `actions-lint` ジョブでも同じ検査（actionlint + pinact）が走ります。
