...
GitHub Actions ワークフローの検査ツール（actionlint / pinact / shellcheck）は [mise](https://mise.jdx.dev/) で管理します（`mise.toml` でバージョン固定）。actionlint は `run:` スクリプトの検査に shellcheck を利用するため、CI（shellcheck プリインストール済み）と同じ指摘をローカルでも得られるよう shellcheck も併せて固定しています。

```sh
mise install            # actionlint / pinact を導入（ローカルと CI で同一バージョン）
...
GitHub Actions は **コミット SHA で pin** しています（タグ移動によるサプライチェーン攻撃対策。バージョンは末尾コメントで追跡）。新しい Action を追加したら `mise run actions:pin` で pin してください。これらのメタ検査（actionlint + pinact）は deno のコード検査（`ci.yml`）とは別の `workflow-lint.yml` ワークフローで、ワークフロー定義か `mise.toml` が変わった時に走ります。

deno 側のタスク（`fmt:check` / `lint` / `check` / `test`）は従来どおり `deno task <name>` で実行します。
