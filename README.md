...
依存ライブラリ（`npm:asana` ほか）はいずれも permissive ライセンス（MIT / ISC / BSD / Apache-2.0）で、コンパイル済みバイナリへの同梱・再配布が可能です。プロジェクト自身と Asana SDK の帰属表示は [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) にまとめており、`release-build.yml` が `LICENSE` とともに Release アセットへ自動同梱します。推移的依存の本文まで網羅した帰属束が必要な配布者向けに、`deno.lock` から完全版を生成する手順も同ファイルに記載しています（ソースから `deno task` で実行する場合は依存がバンドルされないため同梱は不要です）。
