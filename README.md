...
依存ライブラリ（`npm:asana` ほか）はいずれも permissive ライセンス（MIT / ISC / BSD / Apache-2.0）で、コンパイル済みバイナリへの同梱・再配布が可能です。主要依存（Asana SDK）の帰属表示と、推移的依存の帰属を `deno.lock` から完成させる手順は [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) にまとめています。**バイナリを配布する場合は、同ファイル記載の手順で全依存をカバーした完全な帰属表示を生成し、バイナリに同梱してください**（ソースから `deno task` で実行する場合は依存がバンドルされないため不要です）。
